...
class popularity_recommender_py():
    ...
    def create(self, train_data, user_id, item_id):
        ...
        train_data_grouped = train_data.groupby([self.item_id]).agg({'ratings': 'sum'}).reset_index()
        #train_data_grouped.rename(columns = {'user_id': 'score'},inplace=True)
    
        #Sort the songs based upon recommendation score
        train_data_sort = train_data_grouped.sort_values(['ratings', self.item_id], ascending = [0,1])
...
class item_similarity_recommender_py():
    ...
    def construct_cooccurence_matrix(self, user_products, all_products):
        ...
        for i in range(0,len(all_products)):
            #Calculate unique buyers (users) of product (item) i
            ...
            for j in range(0,len(user_products)):       
                ...
                #Get unique buyers (users) of product (item) j
                users_j = user_products_users[j]
                    
                #Calculate intersection of buyers of products i and j
                users_intersection = users_i.intersection(users_j)
                
                #Calculate cooccurence_matrix[i,j] as Jaccard Index
                if len(users_intersection) != 0:
                    #Calculate union of buyers of products i and j
